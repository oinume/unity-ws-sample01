...
describe('WebSocketServer', function() {
  describe('#ctor', function() {
    ...
    it('throws an error if no port or httpServer is specified', function() {
      var gotException = false;
      try {
        var wss = new WebSocketServer({});
      }
    ...
    it('does not throw an error if no port or httpServer is specified, when the noServer option is true', function() {
      var gotException = false;
      try {
    ...
    it('emits an error if http httpServer bind fails', function(done) {
      var wss = new WebSocketServer({port: 1});
      wss.on('error', function() { done(); });
    });

    it('starts a httpServer on a given port', function(done) {
    ...
    it('uses a precreated http httpServer', function (done) {
    ...
    it('uses a precreated http httpServer listening on unix socket', function (done) {
      var srv = http.createServer();
    ...
    it('can have two different instances listening on the same http httpServer with two different paths', function(done) {
      var srv = http.createServer();
      srv.listen(++port, function () {
    ...
    it('cannot have two different instances listening on the same http httpServer with the same path', function(done) {
      var srv = http.createServer();
      srv.listen(++port, function () {
        var wss1 = new WebSocketServer({server: srv, path: '/wss1'});
        try {
  ...
  describe('#close', function() {
    ...
    it('does not close a precreated httpServer', function(done) {
      var srv = http.createServer();
      var realClose = srv.close;
      srv.close = function() {
        should.fail('must not close pre-created httpServer');
      }
      srv.listen(++port, function () {
        var wss = new WebSocketServer({server: srv});
        var ws = new WebSocket('ws://localhost:' + port);
    ...
    it('cleans up websocket data on a precreated httpServer', function(done) {
      var srv = http.createServer();
      srv.listen(++port, function () {
        var wss1 = new WebSocketServer({server: srv, path: '/wss1'})
          , wss2 = new WebSocketServer({server: srv, path: '/wss2'});
  ...
  describe('#handleUpgrade', function() {
    it('can be used for a pre-existing httpServer', function (done) {
      var srv = http.createServer();
      srv.listen(++port, function () {
        var wss = new WebSocketServer({noServer: true});
  ...
  describe('hybi mode', function() {
    describe('connection establishing', function() {
      ...
      it('client detects invalid httpServer protocol', function(done) {
        var wss = new WebSocketServer({port: ++port, handleProtocols: function(ps, cb) {
      ...
      it('client detects no httpServer protocol', function(done) {
        var wss = new WebSocketServer({port: ++port, handleProtocols: function(ps, cb) {
            cb(true); }}, function() {
          var ws = new WebSocket('ws://localhost:' + port, {protocol: 'prot1, prot2'});
          ws.on('open', function(client) {
      ...
      it('client refuses httpServer protocols', function(done) {
        var wss = new WebSocketServer({port: ++port, handleProtocols: function(ps, cb) {
            cb(false); }}, function() {
          var ws = new WebSocket('ws://localhost:' + port, {protocol: 'prot1, prot2'});
          ws.on('open', function(client) {
      ...
      it('httpServer detects invalid protocol handler', function(done) {
        var wss = new WebSocketServer({port: ++port, handleProtocols: function(ps, cb) {
