...
/**
 * Gracefully closes the connection, after sending a description message to the httpServer
 *
 * @param {Object} data to be sent to the httpServer
 * @api public
 */
...
function initAsClient(address, protocols, options) {
  ...
  (isNodeV4 ? agent : req).once('response', function(res) {
    var error = new Error('unexpected httpServer response (' + res.statusCode + ')');
    self.emit('error', error);
    cleanupWebsocketResources.call(this, error);
  });
  (isNodeV4 ? agent : req).once('upgrade', function(res, socket, upgradeHead) {
    if (self.readyState == WebSocket.CLOSED) {
      // client closed before httpServer accepted connection
      self.emit('close');
    ...
    if (typeof serverKey == 'undefined' || serverKey !== expectedServerKey) {
      self.emit('error', 'invalid httpServer key');
      removeAllListeners(self);
      socket.end();
      return;
    }
    ...
    if (!options.value.protocol && serverProt) {
        protError = 'httpServer sent a subprotocol even though none requested';
    } else if (options.value.protocol && !serverProt) {
        protError = 'httpServer sent no subprotocol even though requested';
    } else if (serverProt && protList.indexOf(serverProt) === -1) {
        protError = 'httpServer responded with an invalid protocol';
    }
    if (protError) {
