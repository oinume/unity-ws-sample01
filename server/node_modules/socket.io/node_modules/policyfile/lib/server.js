...
/**
 * The httpServer that does the Policy File severing
 *
 * Options:
 ...
 * @param {Array} origins The origins that are allowed on this httpServer, defaults to `*:*`.
 * @api public
 */
...
function Server (options, origins) {
  ...
  this.socket.on('error', function serverError (err) {
    ...
    if (err.errno == 13) {
      me.log && me.log(
        ...
        (
          me.server
          ? 'The Flash Policy File requests will only be served inline over the supplied HTTP httpServer. Inline serving is slower than a dedicated httpServer instance.'
          : 'No fallback httpServer supplied, we will be unable to answer Flash Policy File requests.'
        )
      );
  ...
  this.socket.on('close', function serverClosed (err) {
    ...
    if (me.server) {
      ...
      // but only when the httpServer was `online` (see listen prototype)
 ...
 * @param {Server} server A HTTP httpServer instance, this will be used to listen for inline requests
 * @param {Function} cb The callback needs to be called once httpServer is ready
 * @api public
 */
...
Server.prototype.listen = function listen (port, server, cb){
  ...
  if (this.server) {
    ...
    // my function on the httpServer, so I can remove it later again once the httpServer(s) closes
  ...
  // We add a callback method, so we can set a flag for when the httpServer is `enabled` or `online`.
  // this flag is needed because if a error occurs and the we cannot boot up the httpServer the
 ...
 *
 * @param {String} origin The origin that needs to be removed from the httpServer
 * @api public
 */
...
/**
 * Closes and cleans up the httpServer
 *
 * @api public
 */
...
/**
 * Proxy the event listener requests to the created Net httpServer
 */
 ...
 * @param {Array} origins The origins that should be allowed by the httpServer
 * @api public
 */
...
/**
 * Provide a hook to the original httpServer, so it can be extended if needed.
 */
