 ...
 * If doing a node build for httpServer-side client, this wrapper is NOT included.
 * @api private
 */
...
var builder = module.exports = function () {
  ...
  files.forEach(function (file) {
    fs.readFile(file, function (err, content) {
      ...
      // pre-wrapper for non-httpServer-side builds
      ...
      // post-wrapper for non-httpServer-side builds
      if (!settings.node) {
          code += wrapperPost;
      }
      ...
      // where designed for a httpServer side env. but only if we don't want to
