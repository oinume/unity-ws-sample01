...
public class WebSocket extends EventDispatcher {
  ...
  private function onSocketIoError(event:IOErrorEvent):void {
    ...
    if (readyState == CONNECTING) {
      message = "cannot connect to Web Socket httpServer at " + url + " (IoError)";
    } else {
      message = "error communicating with Web Socket httpServer at " + url + " (IoError)";
    }
    onError(message);
  }
  ...
  private function onSocketSecurityError(event:SecurityErrorEvent):void {
    ...
    if (readyState == CONNECTING) {
      message =
          "cannot connect to Web Socket httpServer at " + url + " (SecurityError)\n" +
          "make sure the httpServer is running and Flash socket policy file is correctly placed";
    } else {
      message = "error communicating with Web Socket httpServer at " + url + " (SecurityError)";
    }
    onError(message);
  }
  ...
  private function validateHeader(headerStr:String):Boolean {
    ...
    if (!lowerHeader["sec-websocket-origin"]) {
      if (lowerHeader["websocket-origin"]) {
        onError(
          "The WebSocket httpServer speaks old WebSocket protocol, " +
          "which is not supported by web-socket-js. " +
          "It requires WebSocket protocol 76 or later. " +
          "Try newer version of the httpServer if available.");
      } else {
