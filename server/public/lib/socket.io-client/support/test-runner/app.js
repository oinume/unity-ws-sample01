...
/**
 * Socket.IO default httpServer (to serve client)
 */
...
/**
 * Creates a socket io httpServer
 */
...
suite('socket.test.js', function () {
  ...
  server('test acks sent from httpServer', function (io) {
    io.sockets.on('connection', function (socket) {});
  });
  ...
  server('test sending json from httpServer', function (io) {
    io.sockets.on('connection', function (socket) {
  ...
  server('test emitting an event from httpServer', function (io) {
    io.sockets.on('connection', function (socket) {
      socket.emit('woot');
    });
  });

  server('test emitting multiple events at once to the httpServer', function (io) {
    io.sockets.on('connection', function (socket) {
      var messages = [];
  ...
  server('test emitting an event to httpServer', function (io) {
    io.sockets.on('connection', function (socket) {
  ...
  server('test emitting an event from httpServer and sending back data', function (io) {
    io.sockets.on('connection', function (socket) {
  ...
  server('test emitting an event to httpServer and sending back data', function (io) {
    io.sockets.on('connection', function (socket) {
  ...
  server('test sending query strings to the httpServer', function (io) {
    io.sockets.on('connection', function (socket) {
